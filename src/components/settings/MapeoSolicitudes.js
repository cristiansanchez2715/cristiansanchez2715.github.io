...
const eliminarSolicitud = (solicitudId) => {
  ...
  const nuevasSolicitudes = totalSolicitudes.filter((solicitud) => {
    ...
    const url = `https://server-ofrecetusservicios.onrender.com/${solicitudId}`;
  ...
  const reducirSolicitudesEn1 = async () => { 
    try {
      const response = await fetch(`https://server-ofrecetusservicios.onrender.com/${usuarioPresente.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
...
// TRAEER TODAS LAS SOCILITUDES
    const getSolicitudes = async () => {
        try {
          const res = await fetch("https://server-ofrecetusservicios.onrender.com/traerSolicitudes");
