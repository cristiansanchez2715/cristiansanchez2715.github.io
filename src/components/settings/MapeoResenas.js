...
const eliminarResena = (resenaId) => {
  ...
  const nuevasReseñas = totalReseñas.filter((reseña) => {
    ...
    const url = `https://server-ofrecetusservicios.onrender.com/deleteresena/${resenaId}`;
  ...
  const reducirReseñasEn1 = async () => { 
    try {
      const response = await fetch(`https://server-ofrecetusservicios.onrender.com/reducirresena/${usuarioPresente.id}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  const functionResenasOfThisUser = () => {
    ...
    const getReseñas = async () => {
        try {
          const res = await fetch("https://server-ofrecetusservicios.onrender.com/traerResenas");
