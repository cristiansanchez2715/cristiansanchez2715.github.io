...
const aumentarReseñasEn1 = async () => { 
  try {
    const response = await fetch(`https://server-ofrecetusservicios.onrender.com/resena/${usuarioPresente.id}`, {
      method: "POST",
      headers: {
...
useEffect(() => {
const apiGetResenas = 'https://server-ofrecetusservicios.onrender.com/traerResenas'
...
const APISendResena = "https://server-ofrecetusservicios.onrender.com/enviarResenas";
