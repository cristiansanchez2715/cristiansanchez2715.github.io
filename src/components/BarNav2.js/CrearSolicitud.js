...
const getUsuarios = () => {
  fetch("https://server-ofrecetusservicios.onrender.com/traerUsuarios")
  .then(res => res.json())
...
const aumentarSolicitudesEn1 = async () => { 
  try {
    const response = await fetch(`https://server-ofrecetusservicios.onrender.com/solicitud/${usuarioPresente.id}`, {
      method: "POST",
      headers: {
...
// MANDAR DATOS DE LA SOLICITUD AL BACKEND 
    ...
    const apiSendSolicitud = "https://server-ofrecetusservicios.onrender.com/enviarSolicitud"
...
<div  className='logo-container2' id='logo-form' style={{marginLeft: "0px",fontSize: "35px" ,display: "flex", flexDirection: "row", paddingTop: "0px"}}>
  <h2><span style={{color: "red",} }>!Encuentra </span><span style={{color: "blue"}}>Un Servicio¡</span> </h2>
