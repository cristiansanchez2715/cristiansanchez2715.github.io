...
const aumentarServiciosEn1 = async () => { 
  try {
    const response = await fetch(`https://server-ofrecetusservicios.onrender.com/servicio/${usuarioPresente.id}`, {
      method: "POST",
      headers: {
...
const apiSendService = "https://server-ofrecetusservicios.onrender.com/enviarServicio"
...
<div  className='logo-container2' id='logo-form' style={{marginLeft: "0px",fontSize: "35px" ,display: "flex", flexDirection: "row", paddingTop: "0px"}}>
  <h2><span style={{color: "red",} }>!Ofrece tu </span><span style={{color: "blue"}}>SERVICIO</span> </h2>
